...
export class ForgeFlowSearchEngine implements ISearchEngine {
  ...
  async search(query: SearchQuery): Promise<SearchResults> {
    ...
    try {
      ...
      if (cached && Date.now() - cached.timestamp < this.cacheExpiry) {
        this.updateQueryStats(query.query, cached.results.totalMatches, Date.now() - startTime, true);
  ...
  async getAnalytics(startDate: Date, endDate: Date): Promise<SearchAnalytics> {
    ...
    const averageQueryLength = Array.from(this.queryStats.keys())
      .reduce((sum, query) => sum + query.length, 0) / uniqueQueries || 0;

    const topQueries = await this.getPopularQueries(10);

    const averageResponseTime = Array.from(this.queryStats.values())
      .reduce((sum, stats) => sum + (stats.totalResponseTime / stats.count), 0) / uniqueQueries || 0;
    ...
    const averageResults = Array.from(this.queryStats.values())
      .reduce((sum, stats) => sum + (stats.totalResults / stats.count), 0) / uniqueQueries || 0;

    const zeroResultQueries = Array.from(this.queryStats.values())
      .filter(stats => stats.totalResults === 0).length;
  ...
  private prepareSearchOptions(query: SearchQuery): any {
    ...
    return {
      ...
      filters: {
        ...
        categories: Array.isArray(query.category) ? query.category : query.category ? [query.category] : undefined,
        ...
        agentTypes: Array.isArray(query.agentTypes) ? query.agentTypes : query.agentTypes ? [query.agentTypes] : undefined,
  ...
  private async processSearchResults(rawResults: any[], query: SearchQuery): Promise<SearchResult[]> {
    ...
    return rawResults.map((raw, index) => {
      ...
      return {
        ...
        score: enhancedScore,
        rank: index + 1,
        titleSnippet: query.highlightResults ? this.highlightText(raw.title, query.query) : undefined,
        contentSnippets,
        matchedFields,
  ...
  private calculateRelevanceFactors(raw: any, query: SearchQuery, weights: SearchWeights): any {
    ...
    const categoryMatch = metadata.category && query.query.toLowerCase().includes(metadata.category.toLowerCase()) ? 1.0 : 0.0;
    ...
    // Effectiveness boost
    const effectivenessBoost = query.boostEffective && metadata.effectiveness 
  ...
  private calculateEnhancedScore(baseScore: number, relevanceFactors: any, weights: SearchWeights): number {
    return baseScore + 
  ...
  private generateContentSnippets(content: string, query: string, maxLength: number): ContentSnippet[] {
  ...
  private mapToFacetCounts(map: Map<string, number>, selectedValues?: string | string[]): FacetCount[] {
  ...
  private updateQueryStats(query: string, resultCount: number, responseTime: number, fromCache: boolean, error?: Error): void {
