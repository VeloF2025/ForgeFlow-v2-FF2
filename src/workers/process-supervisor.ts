...
import type { ProcessInfo, ProcessHealthStatus, ProcessStatus } from './pid-registry';
import { PidRegistry } from './pid-registry';
import type { ProcessMonitoringData } from './process-monitor';
...
export class ProcessSupervisor extends EventEmitter {
  ...
  public async stopProcess(processId: string, reason?: string): Promise<void> {
    this.logger.info(
      `Stopping supervised process: ${processId} - Reason: ${reason || 'User request'}`,
    );
  ...
  public async restartProcess(processId: string, reason?: string): Promise<string> {
    this.logger.info(
      `Restarting supervised process: ${processId} - Reason: ${reason || 'Manual restart'}`,
    );
    ...
    if (restartCount >= maxRestarts) {
      throw new Error(
        `Maximum restart attempts reached for process: ${processId} (${restartCount}/${maxRestarts})`,
      );
    }
    ...
    try {
      ...
      if (activeProcessIds.length > 0) {
        ...
        const stopPromises = activeProcessIds.map((processId) =>
          this.stopProcess(processId, 'Supervisor shutdown').catch((error) =>
  ...
  private async validateSystemCapacity(): Promise<void> {
    ...
    if (memUsagePercent > 85) {
      throw new Error(`System memory usage too high: ${memUsagePercent.toFixed(1)}% (limit: 85%)`);
    }
    ...
    if (loadPercent > 80) {
      throw new Error(`System CPU load too high: ${loadPercent.toFixed(1)}% (limit: 80%)`);
    }
  }
  ...
  private async prepareExecutionEnvironment(
    ...
    const env: Record<string, string> = {
      ...
      // Resource limits
      FF2_MAX_MEMORY_MB: String(
        options.resourceLimits?.maxMemoryMB || this.config.resourceLimits.maxMemoryMB,
      ),
      FF2_MAX_CPU_PERCENT: String(
        options.resourceLimits?.maxCpuPercent || this.config.resourceLimits.maxCpuPercent,
      ),
        ...
        if (shouldRestart) {
          ...
          try {
            await this.restartProcess(
              processId,
              `Process failed (code: ${code}, signal: ${signal})`,
            );
          } catch (error) {
