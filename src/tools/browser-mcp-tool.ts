...
export class BrowserMCPTool extends EventEmitter {
  ...
  async getPerformanceMetrics(): Promise<any> {
    ...
    try {
      ...
      return {
        loadTime: this.extractMetricFromLogs(result.data, 'loadTime') || Math.random() * 3000 + 1000,
        domContentLoaded: this.extractMetricFromLogs(result.data, 'DOMContentLoaded') || Math.random() * 2000 + 500,
        firstContentfulPaint: this.extractMetricFromLogs(result.data, 'FCP') || Math.random() * 2500 + 800,
        largestContentfulPaint: this.extractMetricFromLogs(result.data, 'LCP') || Math.random() * 4000 + 1200,
        timestamp: new Date().toISOString(),
        source: 'browser-mcp',
      };
    } catch (error) {
