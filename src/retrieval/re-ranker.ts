...
export class OnlineLearningReranker implements ILogisticReranker {
  ...
  constructor(config: RetrievalConfig['reranking']) {
    ...
    this.model = {
      ...
      bias: 0.0,
      features: [
        'titleMatchScore', 'contentMatchScore', 'tagMatchScore', 'categoryMatch',
        'creationDecay', 'modificationDecay', 'usageDecay',
        'wordOverlapRatio', 'cosineSimilarity', 'exactPhraseMatch',
        'agentTypeRelevance', 'projectRelevance', 'userSuccessRate',
        'complexityScore', 'readabilityScore', 'hasCodeExamples',
        'issueRelevance', 'isWorkingHours', 'activeProject',
        'overallRelevance', 'confidenceScore', 'noveltyScore'
      ],
      accuracy: 0.5,
      precision: 0.5,
      recall: 0.5,
  ...
  async rerank(
    ...
    try {
      ...
      throw new RetrievalError(
        'Reranking failed',
        RetrievalErrorCode.RERANKER_FAILED,
        { query: query.query, resultsCount: results.length, error }
      );
    }
  }
  ...
  private async trainWithSGD(trainingExamples: Array<{ features: number[]; label: number }>): Promise<void> {
    const epochs = 100;
    ...
    for (let epoch = 0; epoch < epochs; epoch++) {
      ...
      for (const example of shuffled) {
        ...
        for (let i = 0; i < Math.min(example.features.length, this.model.weights.length); i++) {
          ...
          this.model.weights[i] -= learningRate * (weightGradient + regularization * this.model.weights[i]);
        }
  ...
  private async evaluateModel(validationSet: Array<{ features: number[]; label: number }>): Promise<void> {
    ...
    for (let i = 0; i < pairs.length; i++) {
      for (let j = i + 1; j < pairs.length; j++) {
        if (pairs[i].label !== pairs[j].label) {
          total++;
          if ((pairs[i].label > pairs[j].label && pairs[i].pred > pairs[j].pred) ||
  ...
  private calculateMAP(predictions: number[], labels: number[]): number {
    ...
    let relevantFound = 0;
    let sumPrecision = 0;
    let totalRelevant = labels.filter(l => l > 0.5).length;
