...
import type { IndexEntry } from '../indexing/types.js';
...
export class SmartFeatureExtractor implements IFeatureExtractor {
  ...
  async extractFeatures(query: RetrievalQuery, content: IndexEntry): Promise<FeatureVector> {
    try {
      ...
      const [recency, proximity, affinity, semantic, context] = await Promise.all([
        this.config.enableRecencyFeatures
          ? this.extractRecencyFeatures(content)
          : this.getEmptyRecencyFeatures(),
        this.config.enableProximityFeatures
          ? this.extractProximityFeatures(query, content)
          : this.getEmptyProximityFeatures(),
        this.config.enableAffinityFeatures
          ? this.extractAffinityFeatures(query.context, content)
          : this.getEmptyAffinityFeatures(),
        this.config.enableSemanticFeatures
          ? this.extractSemanticFeatures(content)
          : this.getEmptySemanticFeatures(),
        this.config.enableContextFeatures
          ? this.extractContextFeatures(query.context, content)
          : this.getEmptyContextFeatures(),
      ]);

      // Calculate derived features
      const derived = this.calculateDerivedFeatures(
        basic,
        recency,
        proximity,
        affinity,
      ...
      logger.debug('Features extracted', {
        ...
        featuresEnabled: Object.keys(this.config).filter(
          (key) => (this.config as any)[key] === true,
        ),
      });
  ...
  async extractBatchFeatures(
    ...
    try {
      ...
      const scaledFeatures = this.config.normalizeFeatures ? this.scaleFeatures(results) : results;
  ...
  normalizeFeatures(features: FeatureVector): FeatureVector {
    ...
    if (this.config.scalingMethod === 'minmax') {
      normalized.recency.daysSinceCreated = Math.min(
        normalized.recency.daysSinceCreated / 365,
        1.0,
      );
      normalized.recency.daysSinceModified = Math.min(
        normalized.recency.daysSinceModified / 365,
        1.0,
      );
      normalized.recency.daysSinceLastUsed = Math.min(
        normalized.recency.daysSinceLastUsed / 365,
        1.0,
      );
      ...
      normalized.recency.daysSinceCreated =
        (normalized.recency.daysSinceCreated - this.featureStats.recencyMean) /
        this.featureStats.recencyStd;
      // ... apply to other features
    }

    // Normalize derived features
    normalized.derived.overallRelevance = Math.max(
      0,
      Math.min(1, normalized.derived.overallRelevance),
    );
    normalized.derived.uncertaintyScore = Math.max(
      0,
      Math.min(1, normalized.derived.uncertaintyScore),
    );
  ...
  public extractContextFeatures(context: SearchContext, content: IndexEntry): ContextFeatures {
    // Issue relevance
    const issueRelevance = context.currentIssue
      ? this.calculateIssueRelevance(context.currentIssue, content)
      : 0.5;
  ...
  private calculateTextMatchScore(query: string, text: string): number {
    ...
    for (const queryWord of queryWords) {
      if (
        textWords.some((textWord) => textWord.includes(queryWord) || queryWord.includes(textWord))
      ) {
        matches++;
      }
    }
  ...
  private calculateDomainFit(context: SearchContext, content: IndexEntry): number {
    ...
    if (contentTags && issueDomain.split(' ').some((word) => contentTags.includes(word)))
      return 0.8;
  ...
  private calculateIssueRelevance(
    issue: { title: string; labels: string[]; description: string },
    content: IndexEntry,
  ): number {
    const issueText = (
      issue.title +
  ...
  private calculateUrgencyMatch(context: SearchContext, content: IndexEntry): number {
    ...
    const urgentContent =
      urgentLabels.some((label) => contentTags.includes(label)) ||
  ...
  private cleanupCaches(): void {
    ...
    if (this.semanticCache.size > maxCacheSize) {
      const keysToDelete = Array.from(this.semanticCache.keys()).slice(
        0,
        this.semanticCache.size - maxCacheSize,
      );
