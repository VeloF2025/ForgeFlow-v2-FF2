...
import type { RetrievalQuery, RetrievalResults, RetrievalConfig, UserFeedback } from './types.js';
import { SearchContext, BanditAlgorithm, RetrievalError, RetrievalErrorCode } from './types.js';
...
export class RetrievalIntegrationService {
  ...
  private async getResultMemoryContext(
    results: RetrievalResults,
    query: RetrievalQuery,
  ): Promise<{
    insights: string[];
  ...
  private calculateReward(feedback: UserFeedback): number {
    ...
    // Dwell time bonus
    if (feedback.clicked && feedback.dwellTime > 10000) {
      // 10+ seconds
      reward += 0.1;
    }
    ...
    try {
      ...
      for (const result of results.results) {
        if (result.entry.type === 'knowledge') {
          await this.knowledgeManager.recordUsage(result.entry.id, feedback.queryId, {
            issueId: feedback.queryId,
            success: feedback.usedInSolution || feedback.relevanceRating > 3,
            metrics: {
              timeToResolution: feedback.dwellTime || 0,
              codeQuality: feedback.relevanceRating ? feedback.relevanceRating / 5 : 0.5,
              errorReduction: feedback.usedInSolution ? 1 : 0,
            },
            timestamp: new Date(),
          });
        }
  ...
  private async recordMemoryOutcome(
    ...
    try {
      const outcome = feedback.usedInSolution
        ? 'success'
        : feedback.relevanceRating && feedback.relevanceRating >= 4
          ? 'success'
          : 'partial';
      ...
      await this.memoryManager.recordJobMemory({
        ...
        content: {
          ...
          learnings: feedback.usedInSolution
            ? 'Query was successful and used in solution'
            : 'Query provided partial results',
        },
        outcome,
  ...
  private updateMetrics(totalTime: number, results: RetrievalResults | null, error: boolean): void {
    ...
    if (results && !error) {
      ...
      const avgConfidence =
        results.results.reduce((sum, r) => sum + r.confidenceScore, 0) / results.results.length;
      this.performanceMetrics.averageAccuracy =
        (this.performanceMetrics.averageAccuracy * (this.performanceMetrics.totalQueries - 1) +
          avgConfidence) /
        this.performanceMetrics.totalQueries;
    }
